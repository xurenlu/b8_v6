...
Not much! You just need PHP 5 on the server where b8 will be used and a database. Two backends for storage are currently provided: one using a `Berkeley DB <http://www.oracle.com/database/berkeley-db/index.html>`_ and one using a `MySQL <http://mysql.com/>`_ table.
...
The other thing is that most statistical spam filters count one token one time, no matter how often it appears in the text (as Paul Graham describes it in [#planforspam]_). b8 does count how often a token has been seen and learns resp. considers this. Additionally, the number of learned ham and spam texts is saved and used as the calculation base for the single probabilities. Why this? Because a text containing one link (no matter where it points to, just indicated by a "\h\t\t\p\:\/\/" or a "www.") might not be spam, but a text containing 20 links might be.
...
These are some basic settings telling b8 which backend classes to use:
	...
	**storage**
		This defines which storage backend will be used to save b8's wordlist. Currently, two backends are available: `Berkeley DB <http://www.oracle.com/database/berkeley-db/index.html>`_ (``dba``) and `MySQL <http://mysql.com/>`_ (``mysql``). The default is ``dba`` (string).
		...
		*mysql (MySQL)*
			A MySQL table can also be used for storing b8's wordlist. This backend needs of course a running and accessable MySQL server and database. |br|
...
Each "normal" token is stored with it's literal name as the key and it's data as the value. The backends store the token's data in a different way. The DBA backend simply stores a string containing both values separated by a space character. The MySQL backend stores the counters in different columns.
